    ...
    const fetchChunk = async () => {
      ...
      try {
        ...
        setMetadata({
          ...
          lastReviewed: chunk.lastReviewed ? new Date(chunk.lastReviewed).toISOString().split("T")[0] : "",
  ...
  const handleUpdateContent = async () => {
    ...
    if (uploadType === "file") {
      ...
      if (selectedFile.type === "text/plain") {
        ...
        // For PDF or other types, we might need server-side parsing
        toast.error("File upload for non-text files in Edit mode is not fully implemented yet.");
        return;
      }
    }
    ...
    <div className="mx-auto space-y-8 px-4 pb-12 transition-all duration-500">
      <div className="flex items-center gap-4">
        ...
        <h2 className="mb-0 font-medium">
           {t("upload.edit_document")}
        </h2>
      </div>

      <div className={cn("flex flex-col-reverse gap-8 lg:grid lg:grid-cols-3", isLoading ? "pointer-events-none opacity-40 grayscale-50" : "opacity-100")}>
        {/* Metadata Section */}
